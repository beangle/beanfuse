...
<table width="100%">
  ...
  <form name="actionForm" method="post" action="param.action?method=saveParamGroup">
    <input type="hidden" name="paramGroup.name">
  </form>
 </body>
<script>
   var form = document.actionForm; 
   function addGroup(){
      var name=prompt("请输入参数组名称","");
      if(name!=null && name!=""){
          form['paramGroup.name'].value=name;
          form.submit();
      }
   }
   ...
   function removeGroup(groupId){
       if(confirm("是否确定删除?")){
           form.action="param!removeGroup.action?paramGroupId="+groupId;
           form.submit();
       }
   }
</script>